...
class BangumiRandomPicker {
    ...
    bindEvents() {
        ...
        // 导入想玩
        document.getElementById('import-wishlist-btn').addEventListener('click', () => this.importWishlistGames());
    ...
    // 导入用户“想玩”游戏到自定义池
    async importWishlistGames() {
        if (!this.accessToken) {
            this.showError('请先登录 Bangumi 账号');
            return;
        }

        const username = this.username || localStorage.getItem('bgm_user_id');
        if (!username) {
            this.showError('未能获取当前用户标识，请重新登录后重试');
            return;
        }
        ...
        try {
            ...
            // v0: GET /v0/users/{username}/collections?subject_type=4&type=1
            // 约定：type=1 为 想玩(wish)，subject_type=4 为 游戏
            while (true) {
                const url = `https://api.bgm.tv/v0/users/${encodeURIComponent(username)}/collections?subject_type=4&type=1&limit=${limit}&offset=${offset}`;
                const resp = await fetch(url, { headers });
                if (!resp.ok) {
                    if (resp.status === 401 || resp.status === 403) {
                        throw new Error('获取想玩清单需要登录权限，请重新登录后重试');
                    }
                    throw new Error(`获取想玩清单失败（HTTP ${resp.status}）`);
                }
            ...
            for (const entry of all) {
                ...
                if (!subject || subject.type !== 4) continue;
                if (this.customGamePool.some(g => g.id === subject.id)) continue;
            ...
            if (addedCount > 0) {
                // 复用错误提示容器作为轻量提示
                this.showError(`已导入 ${addedCount} 个“想玩”游戏到自定义池`);
            } else {
                this.showError('“想玩”列表中的游戏已全部在自定义池中');
            }

        } catch (error) {
            console.error('导入想玩失败:', error);
            this.showError(error.message);
        } finally {
            this.showLoading(false);
